...
export function useAnimationLogic(): AnimationContextType {
    ...
    return {
        ...
        setSvgContent: useCallback((val: string | ((prev: string) => string)) => {
            dispatch({ type: 'SET_SVG', payload: { content: val } });
        }, []),
        setSvgContentWithBroadcast,
        setSvgRef,
        setAIProvider: useCallback((p: AIProviderId) => dispatch({ type: 'SET_AI_CONFIG', payload: { provider: p } }), []),
        setAIModel: useCallback((m: string) => dispatch({ type: 'SET_AI_CONFIG', payload: { model: m } }), []),
        setPlaybackSpeed: useCallback((s: number | 'groovy') => dispatch({ type: 'SET_PLAYBACK_SPEED', payload: s }), []),
        setChatHistory: useCallback((val: Message[] | ((prev: Message[]) => Message[])) => dispatch({ type: 'SET_CHAT_HISTORY', payload: val }), []),
        setIsReverse: useCallback((val: boolean) => dispatch({ type: 'SET_REVERSE', payload: val }), []),
        togglePlayPause: useCallback(() => dispatch({ type: 'SET_PLAYING', payload: !state.playback.isPlaying }), [state.playback.isPlaying]),
        ...
        // Utils
        loadPreset: useCallback(async (name: string) => {
            const res = await AnimationApi.getPreset(name);
            dispatch({ type: 'SET_SVG', payload: { content: res.svg } });
            return res.message;
        }, []),
        pauseAnimations: useCallback(() => dispatch({ type: 'SET_PLAYING', payload: false }), []),
        resumeAnimations: useCallback(() => dispatch({ type: 'SET_PLAYING', payload: true }), []),
        resetAnimations: useCallback(() => dispatch({ type: 'RESET_ANIMATION' }), []),
        resetEverything: useCallback(() => dispatch({ type: 'RESET_EVERYTHING' }), []),
        exportAnimation: useCallback((filename: string, format: 'svg' | 'json', includeBackground?: boolean) => {
            exportAnimationUtil(
                state.data.svg,
                filename,
                format,
                state.data.chatHistory,
                viewerPreferences.currentBackground,
                includeBackground
            );
        }, [state.data.svg, state.data.chatHistory, viewerPreferences.currentBackground]),
        canExportAsSvg: useCallback(() => canExportAsSvgUtil(state.data.svg), [state.data.svg])
    };
}
